...
const latest: RequestHandler = async (__, response) => {
  try {
    const responseArray: any[] = [];

    const result = await IGDB.latestGames();
...
const latestGames = (app: Application) => app.get('/games/latest', requireAuthenticated, latest);
